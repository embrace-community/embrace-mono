...
contract AppCreations {
    ...
    function getCollection(uint256 _communityId, uint128 _id) public view returns (Collection memory) {
        uint128 index = _id - 1;
        return communityCollections[_communityId][index];
    }

    function getCollectionCount(uint256 _communityId) public view returns (uint64) {
        return communityToCollectionCount[_communityId];
    }

    function getCollections(uint256 _communityId) public view returns (Collection[] memory) {
        return communityCollections[_communityId];
    }
}
